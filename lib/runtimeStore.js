...
const ALL_MODULE_IDS = MODULES.map((item) => item.id);
...
function normalizeUser(user = {}) {
  ...
  return {
    ...
    moduleAccess: Array.isArray(user.moduleAccess)
      ? user.moduleAccess.filter((id) => ALL_MODULE_IDS.includes(id))
      : ALL_MODULE_IDS,
...
export async function readAuthStore() {
  ...
  return {
    users: Array.isArray(data.users) ? data.users.map(normalizeUser).filter((u) => u.phone) : [],
...
export async function verifyLoginCode(phone, code) {
  ...
  const existing = store.users.find((u) => u.phone === cleanPhone);
  if (existing?.status === "disabled") {
    return { ok: false, error: "这个账号暂时没有访问权限，请联系管理员" };
  }

  delete store.codes[cleanPhone];
  if (!store.users.some((u) => u.phone === cleanPhone)) {
    store.users.push(normalizeUser({ phone: cleanPhone, createdAt: Date.now(), lastLoginAt: Date.now() }));
  } else {
    store.users = store.users.map((u) => u.phone === cleanPhone ? normalizeUser({ ...u, lastLoginAt: Date.now() }) : u);
  }
  await writeAuthStore(store);
...
export async function updateUser(phone, patch = {}) {
  const cleanPhone = normalizePhone(phone);
  if (!isValidChinaMobile(cleanPhone)) return { ok: false, error: "手机号无效" };

  const store = await readAuthStore();
  const found = store.users.find((u) => u.phone === cleanPhone);
  if (!found) return { ok: false, error: "没有找到这个学员" };

  const nextUser = normalizeUser({
    ...found,
    ...(patch.status ? { status: patch.status } : {}),
    ...(patch.role ? { role: String(patch.role) } : {}),
    ...(Array.isArray(patch.moduleAccess) ? { moduleAccess: patch.moduleAccess } : {}),
    ...(typeof patch.note === "string" ? { note: patch.note } : {}),
  });

  store.users = store.users.map((u) => u.phone === cleanPhone ? nextUser : u);
  await writeAuthStore(store);
  return { ok: true, user: nextUser };
}

export async function getUserAccess(phone, moduleId) {
  const cleanPhone = normalizePhone(phone);
  if (!cleanPhone) return { ok: false, error: "请先登录" };
  if (!isValidChinaMobile(cleanPhone)) return { ok: true };

  const store = await readAuthStore();
  const user = store.users.find((u) => u.phone === cleanPhone);
  if (!user) return { ok: false, error: "账号不存在，请重新登录" };
  const normalized = normalizeUser(user);
  if (normalized.status === "disabled") return { ok: false, error: "这个账号暂时没有访问权限，请联系管理员" };
  if (moduleId && !normalized.moduleAccess.includes(moduleId)) {
    return { ok: false, error: "你暂时没有访问这一关的权限，请联系管理员" };
  }
